...
Input includes observation/outcome, authoritative goal/directive, strategy/tactic, native signals, verification, bounded recent decisions with Jev receipts, event details and learning evidence when available. `baseline` contains the observation and outcome from the last supervision, initially the starting observation. Sol supplies a replacement observer in every response, or retains its current source. This lets it express what evidence should require attention next without polling through language-model calls.
