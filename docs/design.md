...
Chess, Snake, and Pac-Man have separate `@gamebot/chess`, `@gamebot/snake`, and `@gamebot/pacman` bridge packages. The bridges depend on `@gamebot/core`; their game rules are not included in core. The chess bridge uses `chess.js`. Snake's seeded game, real-time clock, and browser window live in the independent `snake-game` package, which has no GameBot dependency and can run by itself. The optional Snake bridge only observes state, sends directions, and adds display annotations. Pac-Man follows the same separation through a hosted browser game: JS Pac-Man owns its maze, ghosts, clock, and window, while `@gamebot/pacman` reads its state and sends keys through Playwright. These are initial integration fixtures, not evidence that a particular model or hierarchy plays either game well.
