...
Chess and Snake have separate `@gamebot/chess` and `@gamebot/snake` bridge packages. The bridges depend on `@gamebot/core`; neither rule set is included in core. The chess bridge uses `chess.js`. Snake's seeded game, real-time clock, and browser window live in the independent `snake-game` package, which has no GameBot dependency and can run by itself. The optional Snake bridge only observes state, sends directions, and adds display annotations. These are initial integration fixtures, not evidence that a particular model or hierarchy plays either game well.
