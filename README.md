...
For repeated experiments, run `npm run autoplay -- --game=2048` or `--game=snake`. 2048 accepts `--goal="win"` (default), `--goal="maximize score"`, or a plain-text request mapped to its supported evaluators in both play and autoplay. Score mode continues past 2048; `--policy=latest` selects the saved policy for the chosen goal. The shared researcher evaluates gameplay, proposes competing hypotheses, and revises the complete player: prompts, review intervals, generated JavaScript, or hybrid AI/code decisions. Revisions must pass matched training and fresh validation comparisons, then a final audit before publication. New research starts from its latest evaluated player; add `--fresh` to start from rules again. Resume an interrupted experiment with `--resume=.gamebot/research/<game>/<run-id>`; completed trials, proposals and model usage are retained. Research continues after wins to test efficiency improvements while preserving goal performance. Ordinary play always starts a new AI player unless you pass `--policy=latest` or a specific artifact path. Autoplay opens the game by default. For 2048, it controls the actual browser game with arrow keys and resets the same page between research attempts; strategy and tactic updates appear with `--verbose`. Snake uses its live browser viewer. Use `--headless` to hide the window (2048 still runs in a real browser) or `--pace=0` to remove the viewing delay. See the [learning guide](docs/learning.md) for budgets, evidence, model configuration, and the game integration interface.
