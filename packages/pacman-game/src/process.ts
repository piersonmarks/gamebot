...
import { PacmanEngine } from "./engine.js";
import type { PacmanState, Direction } from "./rules.js";
import { startPacmanViewer, type PacmanPresentation } from "./viewer.js";

const interval = Number(process.argv[2] ?? 140);
...
if (!Number.isSafeInteger(interval) || interval < 1 || initialSeed !== undefined && !Number.isSafeInteger(initialSeed)) {
  throw new Error("Usage: pacman-game [tick interval in ms] [headless] [seed]");
}

let game: PacmanEngine | undefined, gameId: string | undefined, state: PacmanState | undefined;
let closing = false;
const inputSignal = new AbortController();
const send = (message: unknown) => { if (process.connected) process.send?.(message as any, () => {}); };
const observe = async () => {
  if (!game) throw new Error("Pac-Man has no active game");
...
const input = async (action: unknown) => {
  if (!["up", "right", "down", "left"].includes(String(action))) throw new Error("Invalid Pac-Man direction");
  if (!game) throw new Error("Pac-Man has no active game");
  await game.execute(action as Direction, inputSignal.signal);
};
let viewer: Awaited<ReturnType<typeof startPacmanViewer>>;
const close = async () => {
  if (closing) return;
...
const newGame = async (seed: number) => {
  if (!Number.isSafeInteger(seed)) throw new Error("Invalid Pac-Man seed");
  game?.dispose();
  gameId = randomUUID();
  game = new PacmanEngine(seed, interval, next => { state = next; viewer.publish(next); });
...
try {
  viewer = await startPacmanViewer({ observe, input, newGame, close }, !process.connected);
  ...
  process.on("disconnect", () => {
    ...
    if (headless) void close();
      ...
      viewer.allowReset();
      viewer.present({ phase: "Controller disconnected. Pac-Man continues; use arrow keys or Close game." });
    }
  });
  process.on("message", async (message: any) => {
    try {
      if (message.type === "close") { send({ id: message.id, result: null }); await close(); return; }
      if (closing) throw new Error("Pac-Man window is closed");
      if (message.type === "present") { viewer.present(message.presentation as PacmanPresentation); return; }
      if (message.type === "new-game") { send({ id: message.id, result: await newGame(message.seed) }); return; }
      if (message.gameId !== gameId) throw new Error("This Pac-Man game has been replaced");
      ...
      else throw new Error("Unknown Pac-Man request");
    } catch (error) { send({ id: message.id, error: String(error) }); }
  });
  if (process.connected) send({ type: "ready", url: viewer.url });
  else console.log(`Pac-Man is running at ${viewer.url}. Use arrow keys in the browser; Ctrl+C closes it.`);
} catch (error) {
