...
<style>
  ...
  .body{background:#65a30d}.food{background:#fb7185}#status{color:#d1d5db}
</style>
<main><h1>Gamebot Snake</h1><p id="status">Waiting for game…</p><div id="board" aria-label="Snake board"></div></main>
<script>
  const board=document.getElementById('board'), status=document.getElementById('status');
  new EventSource('/events').onmessage=({data})=>{
    const state=JSON.parse(data), body=state.body, head=body[0];
    board.style.gridTemplateColumns='repeat('+state.width+',36px)';
    board.replaceChildren();
    for(let y=0;y<state.height;y++)for(let x=0;x<state.width;x++){
      const cell=document.createElement('div');cell.className='cell';
      if(head.x===x&&head.y===y)cell.classList.add('head');
      else if(body.slice(1).some(p=>p.x===x&&p.y===y))cell.classList.add('body');
      else if(state.food?.x===x&&state.food.y===y)cell.classList.add('food');
      board.append(cell);
    }
    status.textContent='Tick '+state.tick+' · food '+state.foodEaten+(state.alive?'':' · collision');
  };
</script></html>`;
