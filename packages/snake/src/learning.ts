...
export function learningSnake(session: SnakeSession): LearningGame<SnakeState, Direction> {
  const { target, tickIntervalMs } = session;
  return {
...
The native game ends only on collision or when the board is full. Reach the user's food target without a collision to satisfy the evaluation goal; it does not pause the native game.`,
    ...
    // Food only grows before a collision, so reaching the target remains an achieved goal.
    outcome: state => ({ done: !state.alive || state.foodEaten >= target, won: state.foodEaten >= target, score: state.foodEaten }),
  };
}
