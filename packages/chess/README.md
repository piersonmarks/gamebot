...
The runner creates a writable `.gamebot/games/chess/tools/` directory at the GameBot repository root when using `npm run game` or `npm run chess`, for agent-authored drafts. The package's [`tools/`](tools) directory is reserved for reviewed game-specific implementations. A draft may analyze chess state or propose a move, but it does not become callable merely by being placed there; the bridge must register it explicitly.
