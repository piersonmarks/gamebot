...
Policies and full journals are saved under `.gamebot/research/2048/`; `.gamebot/games/2048/goals/<goal-key>/latest-player.json` stores the selected evaluated player separately for each goal and target. These paths are relative to the package directory under the unified launcher. Ordinary play never loads research implicitly. See the [shared learning guide](../../docs/learning.md) for the policy code contract, execution limits, model configuration and extension interface.
