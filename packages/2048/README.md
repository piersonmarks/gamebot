...
On first run, the bridge downloads a pinned copy of the original game into your user cache. It reuses Playwright's browser if present, then looks for installed Chrome, Edge, or Chromium before downloading Playwright Chromium. Neither the game nor a browser is bundled into Gamebot. You can use an existing game checkout instead with `--game-dir=/path/to/2048` or `GAMEBOT_2048_DIR`.
