...
/** Controls a standalone local game through its small process message interface. */
export class LocalGameSession {
  ...
  constructor(readonly tickIntervalMs: number, private readonly processUrl: string, private readonly gameName: string) {
    if (!Number.isSafeInteger(tickIntervalMs) || tickIntervalMs < 1) throw new Error(`${gameName} --pace must be a positive tick interval in milliseconds`);
  }

  open(headless = true): Promise<void> {
    if (this.closing) return Promise.reject(new Error(`${this.gameName} window is closed`));
    return this.opening ??= new Promise<void>((resolve, reject) => {
      let ready = false;
      const child = this.child = fork(new URL(this.processUrl), [String(this.tickIntervalMs), String(headless)],
      ...
      child.on("exit", code => failed(new Error(`${this.gameName} process exited (${code})`)));
      child.on("disconnect", () => failed(new Error(`${this.gameName} connection closed`)));
  ...
  protected async createGame(seed: number): Promise<string> {
    await this.open();
    return this.request<string>({ type: "new-game", seed });
  }

  async request<T>(message: Record<string, unknown>, signal?: AbortSignal): Promise<T> {
    signal?.throwIfAborted();
    if (!this.child?.connected) throw new Error(`${this.gameName} is not connected`);
